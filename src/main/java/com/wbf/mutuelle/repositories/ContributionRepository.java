...
public interface ContributionRepository extends JpaRepository<Contribution, Long> {
    ...
    /**
     * Récupère le montant total des cotisations d'un membre par période pour les graphiques
     * Retourne: [periodId, periodName, totalAmount]
     */
    @Query("SELECT c.contributionPeriod.id, c.contributionPeriod.name, COALESCE(SUM(c.amount), 0) " +
            "FROM Contribution c " +
            "WHERE c.member.id = :memberId OR :memberId IN (SELECT m.id FROM c.members m) " +
            "GROUP BY c.contributionPeriod.id, c.contributionPeriod.name " +
            "ORDER BY c.contributionPeriod.id DESC")
    List<Object[]> findAmountByPeriodForMember(@Param("memberId") Long memberId);
    ...
    List<Contribution> findRecentContributionsByMemberId(@Param("memberId") Long memberId, Pageable pageable);
